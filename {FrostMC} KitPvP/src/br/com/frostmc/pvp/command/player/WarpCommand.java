			...
			if (args.length == 1) {
				...
				try {
					...
					if(args[0].toLowerCase().equals("1v1")) {
						...
						sendError(player, "A warp citada não existe.");
						return true;
					} else {
						sendError(player, "A warp citada não existe.");
						return true;
				...
				if(WarpsAPI.isInWarp(player, warp)) {
					player.sendMessage("§8[§c§lERRO§8] §fVocê já está nessa §aWARP§7.");
					return true;
				}
				WarpManager.send(player, warp);
				player.sendMessage(" ");
				player.sendMessage("§8[§e§lWARP§8] §fVocê está sendo enviado para warp §a" +  Warps.valueOf(warp.name().toLowerCase()) + "§f.");
				player.sendMessage(" ");
			}
			return true;
		} else {
