...
public class SetwarpCommand extends BaseCommand {
	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (args.length == 0) {
				player.sendMessage("");
				player.sendMessage("§8[§b§lADMINISTRAÇÃO§8]");
			...
			try {
				if (args[0].equalsIgnoreCase("1v1")) {
					...
					player.sendMessage("§8[§b§lADMINISTRAÇÃO§8] §fVocê setou a warp §a§l" + warp.name().toLowerCase() + "§f!");
					return true;
				}
				...
				try {
					...
					player.sendMessage("§8[§b§lADMINISTRAÇÃO§8] §fVocê setou o local §a" + local.name().toLowerCase() + "§f!");
					return true;
