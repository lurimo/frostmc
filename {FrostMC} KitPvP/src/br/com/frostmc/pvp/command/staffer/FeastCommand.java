		...
		if (isPlayer(commandSender)) {
			...
			if (args.length == 1) {
				...
				if (args[0].equalsIgnoreCase("force")) {
					player.sendMessage("§8[§a§lFEAST§8] §fVocê forçou o spawn do feast. Tome cuidado ao utilizar esta ferramenta para não prejudicar os jogadores.");
					FrostPvP.getFeast().tempo = 1;
					return true;
				} else if (args[0].equalsIgnoreCase("pos1")) {
					FrostPvP.getConfigManager().registerInConfig(player, "Feast.pos1");
					player.sendMessage("§8[§a§lFEAST§8] §fVocê setou a primeira localização do feast: §f(§6" + player.getLocation().getBlockX() + "§f, §6" + player.getLocation().getBlockY() + "§f, §6" + player.getLocation().getBlockZ() + "§f).");
					return true;
				} else if (args[0].equalsIgnoreCase("pos2")) {
					if (FrostPvP.getConfigManager().getLocationFromConfig("Feast.pos1") == null) {
						player.sendMessage("§8[§a§lFEAST§8] §fVocê ainda não setou a primeira localização.");
						return true;
					}
					FrostPvP.getConfigManager().registerInConfig(player, "Feast.pos2");
					player.sendMessage("§8[§a§lFEAST§8] §fVocê setou a segunda localização do feast:§f (§6" + player.getLocation().getBlockX() + "§f, §6" + player.getLocation().getBlockY() + "§f, §6" + player.getLocation().getBlockZ() + "§f).");
					return true;
	...
	private void setCompassTarget(Player player) {
		if (WarpsAPI.isInWarp(player, Warps.ARENA)) {
			if (player.getInventory().contains(Material.COMPASS))  {
				...
				player.sendMessage("§8[§a§lFEAST§8] §fBússola agora indicando a direção do feast.");
			} else {
				player.sendMessage("§8[§c§lERRO§8] §fVocê não possui uma bússola no inventário.");
			}
		} else {
			player.sendMessage("§8[§c§lERRO§8] §fVocê não está jogando na warp arena, logo, nenhum feast disponível.");
		}
	}
