	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (args.length == 1) {
				...
				if (targeton == null) {
					player.sendMessage("§8[§c§lADMIN§8] §fO jogador especificado não se encontra jogando na network.");
					return true;
				}
				if (targeton.equals(player)) {
					sendError(player, "Você não pode interagir com si mesmo.");
					return true;
				}
				player.openInventory(targeton.getInventory());
				player.sendMessage("§8[§c§lADMIN§8] §fVocê abriu o inventário do jogador " + player.getName() + "§f.");
				return true;
			}
		} else {
			return true;
