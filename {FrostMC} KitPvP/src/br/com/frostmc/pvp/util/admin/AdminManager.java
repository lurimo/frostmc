...
public class AdminManager {
	...
	public static void setAdmin(Player player) {
		...
		player.sendMessage("§8[§c§lADMIN§8] §fVocê agora está em modo de admin.");
		player.sendMessage("§8[§c§lADMIN§8] §fVocê está invisível para " + getInvisible(player) + "");
		
		player.getInventory().setItem(0, new ItemBuilder().setNome(ManagerInventory.warpSelector).setMaterial(Material.PAPER).finalizar());
		player.getInventory().setItem(1, new ItemBuilder().setNome("§3Informações - §7Clique no jogador").setMaterial(Material.NAME_TAG).finalizar());
	...
	public static void setPlayer(Player player) {
		...
		if (player.isOnline()) {
			...
			player.sendMessage("§8[§c§lADMIN§8] §fVocê saiu do modo admin.");
			player.sendMessage("§8[§c§lADMIN§8] §fVocê está visível para §7§lMEMBROS");
		}
