...
public static String titleStoreKits;
	...
	public static void abrirMenu(Player p) {
		...
		inv.setItem(4, new ItemBuilder().setMaterial(Material.NETHER_STAR).setNome("§9Informação:").setLore(Arrays.asList("", "§fEscolha uma habilidade para", "§fcomprar com suas fichas.", "", "§fOBS: §a§nO preço de todos os", "§a§nkits são 1 FICHA.", "", "§fFichas: §e" + account.getStatsGlobal().getFichas())).finalizar());
	...
	public static void addKits(Player p, Inventory inv) {
		...
		if (id > 42)
			inv.setItem(35, new ItemBuilder().setMaterial(Material.ARROW).setNome("§aPróxima página").glow().finalizar());
		if (id == 16) {
			inv.setItem(31, new ItemBuilder().setMaterial(Material.REDSTONE).setNome("§cVocê possuí todos os Kits").finalizar());
		}
	}
	...
	public static void proximaPagina(Player p, Inventory inv) {
		...
		int id = 16;
		inv.setItem(35, new ItemBuilder().setMaterial(Material.ARROW).setNome("§cNenhuma página").finalizar());
		ArrayList<Integer> idsSetados = new ArrayList<>();
		ArrayList<String> names = new ArrayList<>();
		for (Kits kits : Kits.values()) {
			if (!names.contains(kits.name())) {
