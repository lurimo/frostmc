package br.com.frostmc.hardcoregames.util.boosbar;
...
import br.com.frostmc.hardcoregames.util.Util;
import br.com.frostmc.hardcoregames.util.boosbar.dragon.FakeDragon;
...
public class BarAPI {
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(FrostHG.getInstance(), new Runnable() {
	...
	public static void setMessage(final Player player, String message, int seconds) {
		...
		timers.put(player.getUniqueId(), Bukkit.getScheduler().runTaskLater(FrostHG.getInstance(), new Runnable() {
			@Override
			public void run() {
				removeBar(player);
				cancelTimer(player);
