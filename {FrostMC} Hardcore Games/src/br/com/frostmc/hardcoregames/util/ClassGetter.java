...
public class ClassGetter {
	...
	public static void loadCommand() {
		for (Class<?> classes : ClassGetter.getClassesForPackage(FrostHG.getPlugin(FrostHG.class), "br.com.frostmc.hardcoregames.commands")) {
			try {
				if (BaseCommand.class.isAssignableFrom(classes) && classes != BaseCommand.class) {
					BaseCommand baseCommand = (BaseCommand) classes.newInstance();
					((CraftServer) Bukkit.getServer()).getCommandMap().register(baseCommand.getName(), baseCommand);
				}
	...
	public static void loadListener() {
		for (Class<?> classes : ClassGetter.getClassesForPackage(FrostHG.getPlugin(FrostHG.class), "br.com.frostmc.hardcoregames")) {
			try {
				if (Listener.class.isAssignableFrom(classes)) {
					Listener listener = (Listener) classes.newInstance();
					Bukkit.getPluginManager().registerEvents(listener, FrostHG.getPlugin(FrostHG.class));
				}
			} catch (Exception exception) {
				exception.printStackTrace();
			}
