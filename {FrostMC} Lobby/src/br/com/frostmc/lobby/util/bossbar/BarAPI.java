package br.com.frostmc.lobby.util.bossbar;
...
import br.com.frostmc.lobby.util.bossbar.dragon.FakeDragon;
...
public class BarAPI {
	...
	private void handleTeleport(final Player player, final Location loc) {
		...
		Bukkit.getScheduler().runTaskLater(FrostLobby.getInstance(), new Runnable() {
	...
	public static void setMessage(final Player player, String message, int seconds) {
		...
		timers.put(player.getUniqueId(), Bukkit.getScheduler().runTaskLater(FrostLobby.getInstance(), new Runnable() {
			@Override
			public void run() {
				removeBar(player);
				cancelTimer(player);
