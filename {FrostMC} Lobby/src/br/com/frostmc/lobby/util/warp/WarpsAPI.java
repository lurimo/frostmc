package br.com.frostmc.lobby.util.warp;
...
public class WarpsAPI implements Listener{
	...
	public WarpsAPI() {
		Plugin plugin = FrostLobby.getInstance();
	...
	public static void goToWarp(Player p, Warps warp) {
		if(kWarps.contains("Warps." + warp.toString().toUpperCase())) {
			...
			p.sendMessage("§c§lERRO §7Esta warp ainda não foi setada, favor contatar um membro diretivo da equipe.");
		}
	}
