...
public class ClassGetter {
	...
	public static void loadCommandBukkit() {
		for (Class<?> classes : ClassGetter.getClassesForPackage(FrostLobby.getPlugin(FrostLobby.class), "br.com.frostmc.lobby.commands")) {
			try {
	...
	public static void loadListenerBukkit() {
		for (Class<?> classes : ClassGetter.getClassesForPackage(FrostLobby.getPlugin(FrostLobby.class), "br.com.frostmc.lobby")) {
			try {
				if (Listener.class.isAssignableFrom(classes)) {
					Listener listener = (Listener) classes.newInstance();
					Bukkit.getPluginManager().registerEvents(listener, FrostLobby.getPlugin(FrostLobby.class));
				}
			} catch (Exception e) {
			}
		}
