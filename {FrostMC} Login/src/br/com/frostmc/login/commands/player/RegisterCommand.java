	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (!FrostLogin.checkToLogin.get(player.getUniqueId()).equals(AuthType.NONREGISTERED)) {
				player.sendMessage("§8[§c§lERRO§8] §fEste nome de usuário já possui uma conta no servidor, utilize §a/login <senha>§f.");
				return true;
			}
			if (args.length <= 1) {
			...
			if (args.length == 2) {
				if (args[0].length() < 5) {
					player.sendMessage("§8[§c§lERRO§8] §fPara sua segurança, sua senha deve conter no mínimo 6 digitos.");
					return true;
				}
				if (args[1].equalsIgnoreCase(args[0])) {
					for (String block : registerBlocks) {
						if (args[0].contains(block)) {
							player.sendMessage("§8[§c§lERRO§8] §fEsta senha é muito simples, tente algo mais complexo e/ou pessoal.");
							return true;
						}
					}
					...
					if (BukkitMain.getPermissions().isTrial(player)) {
						...
						player.sendMessage("§8[§b§lLOGIN§8] §fSeu codigo de segurança é: " + authentication.getSecurityCode());
					}
