	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (FrostLogin.checkToLogin.get(player.getUniqueId()).equals(AuthType.LOGGED)) {
				player.sendMessage("§8[§c§lERRO§8] §fSua conta já está logada.");
				return true;
			}
			if (args.length == 0) {
				if (BukkitMain.getPermissions().isTrial(player)) {
					...
					player.sendMessage("");
					player.sendMessage("§8[§b§lLOGIN§8] §fBem-vindo de volta! Utilize o comando: §a/login (senha) (codigo de segurança)");
				} else {
					player.sendMessage("§b§lFROST§f§lMC");
					player.sendMessage("");
			...
			if (args.length == 1)  {
				if (!BukkitMain.getPermissions().isTrial(player)) {
					if(args[0].equalsIgnoreCase((String) authentication.getPassword())) {
						...
						player.sendMessage("§8[§b§lLOGIN§8] §fVocê efetuou o login com sucesso.");
						player.sendMessage("§8[§b§lLOGIN§8] §fUtilize a bússola para acessar o lobby.");
						return true;
					} else {
						...
						player.sendMessage("§8[§c§lERRO§8] §fA senha inserida não corresponde com o nosso banco de dados. Tente novamente.");
					}
				} else {
					player.sendMessage("§8[§b§lLOGIN§8] §fUtilize o comando: §a/login " + args[0] + " (codigo de segurança)");
					player.sendMessage("§8[§c§lERRO§8] §fDigite seu codigo de segurança.");
					return true;
				}
			}
			if (args.length == 2) {
				if (BukkitMain.getPermissions().isTrial(player)) {
					if (args[1].equalsIgnoreCase((String) authentication.getSecurityCode())) {
						if(args[0].equalsIgnoreCase((String) authentication.getPassword())) {
							...
							errors.put(player.getUniqueId(), errors.get(player.getUniqueId()) + 1);
							player.sendMessage("§c§lERRO §f7A senha inserida não corresponde com o nosso banco de dados. Tente novamente.");
						}
					} else {
						if (!args[1].contains("#")) {
							player.sendMessage("§8[§c§lERRO§8] §fVocê deve inserir uma §aHASHTAG §fno começo do seu código de segurança.");
							player.sendMessage("§8[§b§lLOGIN§8] §fExemplo: #12345");
							return true;
						} else {
							player.sendMessage("§8[§c§lERRO§8] §fSeu codigo de segurança está incorreto. Tente novamente.");
							return true;
						}
					}
				}
