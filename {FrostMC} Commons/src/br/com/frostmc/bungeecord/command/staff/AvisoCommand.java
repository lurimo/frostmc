	...
	public void execute(CommandSender commandSender, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (args.length == 0) {
				...
				player.sendMessage("");
				player.sendMessage("§f/aviso rec (server/all) §8- §7Aviso para gravação.");
				...
				if (args[0].equalsIgnoreCase("rec")) {
					if (args[1].equalsIgnoreCase("server")) {
						for(ProxiedPlayer all : ProxyServer.getInstance().getPlayers()) {
							...
							if(all.getServer().getInfo().equals(player.getServer().getInfo())) {
								all.sendMessage(" ");
								all.sendMessage("§e§lANÚNCIO §fOlá jogadores, " + playerGroup.fullName() + player.getName() + " §firá §b§lGRAVAR §fnesse servidor.");
								all.sendMessage(" ");
							}
						}
						return;
						...
						GroupsType playerGroup = PlayerLoginListener.checkGroup.get(player.getName());
						BungeeCord.getInstance().broadcast("§e§lANÚNCIO §fOlá jogadores, " + playerGroup.fullName() + player.getName() + " §firá §b§lGRAVAR §fno servidor §e§l" + player.getServer().getInfo().getName().toUpperCase() + " §3§l/connect " + player.getServer().getInfo().getName().toLowerCase().replace("-", "_"));
						return;
					}
				} else if (args[0].equalsIgnoreCase("live")) {
					GroupsType playerGroup = PlayerLoginListener.checkGroup.get(player.getName());
					BungeeCord.getInstance().broadcast("§e§lANÚNCIO §fOlá jogadores, " + playerGroup.fullName() + player.getName() + " §festa §b§lAO VIVO §fem §3" + args[1]);
					return;
					...
					if (args[1].equalsIgnoreCase("server")) {
						for(ProxiedPlayer all : ProxyServer.getInstance().getPlayers()) {
							if(all.getServer().getInfo().equals(player.getServer().getInfo())) {
								GroupsType playerGroup = PlayerLoginListener.checkGroup.get(player.getName());
								all.sendMessage("§e§lANÚNCIO §fOlá jogadores, " + playerGroup.fullName() + player.getName() + " §firá §b§lJOGAR §fnesse servidor.");
								return;
							}
						}
					} else if (args[1].equalsIgnoreCase("all")) {
						GroupsType playerGroup = PlayerLoginListener.checkGroup.get(player.getName());
						BungeeCord.getInstance().broadcast("§e§lANÚNCIO §fOlá jogadores, " + playerGroup.fullName() + player.getName() + " §firá §b§lJOGAR §fno servidor §e§l" + player.getServer().getInfo().getName().toUpperCase() + " §3§l/connect " + player.getServer().getInfo().getName().toLowerCase().replace("-", "_"));
						return;
					}
				}
			}
			...
			if (args[0].equalsIgnoreCase("server")) {
				for(ProxiedPlayer all : ProxyServer.getInstance().getPlayers()) {
					if(all.getServer().getInfo().equals(player.getServer().getInfo())) {
						all.sendMessage("§e§lANÚNCIO §f" + ChatColor.translateAlternateColorCodes('&', message));
					}
				}
				return;
			} else if (args[0].equalsIgnoreCase("all")) {
				BungeeCord.getInstance().broadcast("§e§lANÚNCIO §f" + ChatColor.translateAlternateColorCodes('&', message));
				return;
			}
		} else {
			CommandSender player = commandSender;
			if (args.length == 0) {
				player.sendMessage("§e§lANÚNCIO §fUtilize o comando: /aviso (message)");
				return;
			}
			String message = "";
