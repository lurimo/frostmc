...
import br.com.frostmc.common.permission.injector.CraftBukkitInterface;
		...
		try {
			...
			if (!success) {
				this.plugin.getPlugin().getLogger()
						.warning("Unable to inject PEX's permissible for " + player.getName());
			}
			permissible.recalculatePermissions();
		} catch (Throwable e) {
			this.plugin.getPlugin().getLogger().log(Level.SEVERE,
					"Unable to inject permissible for " + player.getName(), e);
		}
	}
