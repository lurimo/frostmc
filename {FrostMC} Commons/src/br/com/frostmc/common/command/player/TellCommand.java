		...
		if (isPlayer(commandSender)) {
			...
			if (args.length == 1) {
				...
				try {
					...
					player.sendMessage("§8[§c§lERRO§8] §fEste modo não foi identificado.");
					return true;
				}
				if (modes.equals(TellModes.ON)) {
					if (tellModes.get(player.getUniqueId()).equals(TellModes.ON)) {
						player.sendMessage("§8[§e§lTELL§8] §fO seu tell já esta habilitado.");
						return true;
					}
					...
					if (tellModes.get(player.getUniqueId()).equals(TellModes.OFF)) {
						player.sendMessage("§8[§e§lTELL§8] §fO seu tell já esta desabilitado.");
						return true;
					}
			...
			if (tellModes.get(player.getUniqueId()).equals(TellModes.OFF)) {
				player.sendMessage("§8[§c§lERRO§8] §fVocê está com o tell desativado, ative-o para mandar mensagens.");
				return true;
			}
			if (tellModes.get(target.getUniqueId()).equals(TellModes.OFF)) {
				player.sendMessage("§8[§c§lERRO§8] §fEsse jogador está com o tell desabilitado.");
				return true;
			}
