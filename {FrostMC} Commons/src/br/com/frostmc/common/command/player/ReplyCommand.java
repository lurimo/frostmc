	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (args.length == 1) {
				...
				if (target == null) {
					player.sendMessage("§8[§e§lTELL§8] §fNinguém te enviou mensagens recentemente.");
					return true;
				}
				if (target.getName() == null) {
					player.sendMessage("§8[§c§lERRO§8] §fEste jogador não foi encontrado em nossa rede.");
					return true;
				}
				String message = "";
				...
				for (String s : PlayerChatListener.tellBlocks) {
					if (message.toLowerCase().contains(s.toLowerCase())) {
						sendWarning("§8[G-TELL] §7Mensagem dê §f" + player.getName() + " §7para §f" + target.getName() + " §7" + message);
					}
				}
