		...
		if (isPlayer(commandSender)) {
			...
			if (args.length == 0) {
				...
				if (account.getDoubleXP().getRunningType().equals(RunningType.DESACTIVED)) {
					if (account.getDoubleXP().getQuantity() >= 1) {
						...
						player.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê não possui nenhum DOUBLEXP ativo no momento.");
						return true;
					}
				} else {
					if (gamer.isDoublexp()) {
						player.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê já possui um pacote de DOUBLEXP ativo no momento.");
						return true;
					}
				}
			}
			...
			if (args.length == 1) {
				...
				if (target == null) {
					...
					if (!accountOffline.getDoubleXP().exists()) {
						player.sendMessage("§8[§c§lERRO§8] §fEsse jogador não foi encontrado em nosso banco de dados.");
						return true;
					}
			...
			if (args.length == 3) {
				...
				try {
					...
					player.sendMessage("§8[§6§lDOUBLEXP§8] §fEsse modo não foi identificado.");
					return true;
				}
				Player target = Bukkit.getPlayer(args[0]);
				if (target == null) {
					...
					if (modes.equals(DoublexpModes.ADD)) {
						if (isInteger(args[2])) {
							...
							player.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê adicionou §b" + args[2] + " §fquantidade de xp para " + offlinePlayer.getName());
							return true;
						}
						...
						if (isInteger(args[2])) {
							...
							player.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê removeu §b" + args[2] + " §fquantidade de xp dê " + offlinePlayer.getName());
							return true;
						}
					}
					return true;
				...
				if (modes.equals(DoublexpModes.ADD)) {
					if (isInteger(args[2])) {
						...
						player.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê adicionou §b" + args[2] + " §fquantidade de xp para " + target.getName());
						target.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê ganhou §b" + args[2] + " §fquantidade de xp.");
						return true;
					}
				} else if (modes.equals(DoublexpModes.REMOVE)) {
					if (isInteger(args[2])) {
						...
						player.sendMessage("§8[§6§lDOUBLEXP§8] §fVocê removeu §b" + args[2] + " §fquantidade de xp dê " + target.getName());
