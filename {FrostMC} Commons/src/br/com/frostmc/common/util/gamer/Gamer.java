...
public class Gamer {
	...
	public void removeMoedas(int moedas) {
		...
		Player player = Bukkit.getPlayer(this.uuid);
		player.sendMessage("§6§lMOEDAS §fVocê perdeu §6§l" + NumberFormat.getInstance().format(moedas) + " MOEDA" + (moedas < 1 ? "" : "S"));
	}
	public void removeFichas(int fichas) {
		...
		Player player = Bukkit.getPlayer(this.uuid);
		player.sendMessage("§3§lFICHAS §fVocê perdeu §3§l" + NumberFormat.getInstance().format(fichas) + " FICHA" + (fichas < 1 ? "" : "S"));
	}
	...
	public void checkRank() {
		...
		if (allowNextRank()) {
			...
			update();
			Player player = Bukkit.getPlayer(this.uuid);
			player.sendMessage("§6§lLEAGUE §fParabéns, você §a§lUPOU §fsua §6§lLIGA! §fAgora você é um: " + getLeagueType().fullName());
			MessagesType.sendTitleMessage(player, "§6§lLeague", "§fVocê upou para " + getLeagueType().fullName());
			Tags.updateTag(player);
		}
	}
