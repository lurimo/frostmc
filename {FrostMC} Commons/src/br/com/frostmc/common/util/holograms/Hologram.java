package br.com.frostmc.common.util.holograms;
...
import br.com.frostmc.core.util.Value;
...
public class Hologram {
	...
	public Hologram(String text, Location location, boolean register) {
		...
		TIntObjectMap<WatchableObject> dataValues = (TIntObjectMap<WatchableObject>) Value.getValue("dataValues", dataWatcher);
		...
		for (int i = 0; i < 100; i++) {
			...
			int index = (int) Value.getValue("b", current);

			if (index == 2) {
				Value.setValue("c", current, getText());
			} else if (index == 3) {
				Value.setValue("c", current, getText().isEmpty() ? (byte) 0 : (byte) 1);
			} else {
		...
		Value.setValue("b", spawnArmorStand1_8, 30);
