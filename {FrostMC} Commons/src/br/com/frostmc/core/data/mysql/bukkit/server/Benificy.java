...
public class Benificy {
	...
	public static void create(RunningType runningType, BeneficyType beneficyType) {
		try {
			PreparedStatement preparedStatement = BukkitMain.getCore().getMySQL().getConnection().prepareStatement("SELECT * FROM `server_beneficy` WHERE `running` = '" + RunningType.ACTIVED + "';");
