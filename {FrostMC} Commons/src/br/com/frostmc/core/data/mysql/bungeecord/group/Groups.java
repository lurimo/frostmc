...
public class Groups {
	...
	public void setGroupConsole(AccountBungee playerBungee, CommandSender staffer, GroupsType groupType, TemporaryType temporaryType, long expire) {
		...
		ProxyServer.getInstance().getConsole().sendMessage("§a§lGROUP §fVocê alterou o cargo do jogador §a" + playerBungee.getGroups().player + " §fpara " + groupType.fullName() + (temporaryType.equals(TemporaryType.PERMANENT) ? "§fsem data de expiração" : "§firá expirar em " + Longs.messageSmall(expire)));
