...
public class Punishs {
	...
	public void setPunishServer(AccountBungee punidoBungee, AccountBungee stafferBungee, BanType banType, TemporaryType temporaryType, String reason, long expire) {
		ProxiedPlayer stafferPlayer = ProxyServer.getInstance().getPlayer(stafferBungee.getPlayer());
		if (punidoBungee.getPlayer() == null) {
			if (punidoBungee == stafferBungee) {
				stafferPlayer.sendMessage("§c§lPUNISH §fVocê não pode punir a si mesmo.");
				return;
			}
			
			if (punidoBungee.getGroups().hasGroupPermission(stafferBungee.getGroups().getGroupsType())) {
				stafferPlayer.sendMessage("§c§lPUNISH §fVocê não pode punir esse jogador.");
				return;
			}
			
			this.player = punidoBungee.getGroups().player;
			this.ipAddrees = punidoBungee.getGroups().getIpAddrees();
			...
			new PlayerLoginListener().blacklistedAddress.add(punidoBungee.getGroups().getIpAddrees());
			...
			if (banType.equals(BanType.BANNED)) {
				if (temporaryType.equals(TemporaryType.PERMANENT)) {
					stafferPlayer.sendMessage("§4§lBAN §fVocê §4§lBANIU §fo jogador " + stafferPlayer.getName() + " §4§lPERMANENTEMENTE §fpelo motivo " + reason + " §fsem data de expiração.");
				} else {
					stafferPlayer.sendMessage("§4§lTEMPBAN §fVocê §4§lBANIU §fo jogador " + stafferPlayer.getName() + " §4§lTEMPORARIAMENTE §fpelo motivo " + reason + " §firá expirar em " + Longs.messageSmall(expire));
				}
			} else if (banType.equals(BanType.MUTATED)) {
				if (temporaryType.equals(TemporaryType.PERMANENT)) {
					stafferPlayer.sendMessage("§3§lMUTE §fVocê §3§lMUTOU §fo jogador " + stafferPlayer.getName() + " §3§lPERMANENTEMENTE §fpelo motivo " + reason + " §fsem data de expiração.");
				} else {
					stafferPlayer.sendMessage("§3§lTEMPMUTE §fVocê §3§lMUTOU §fo jogador " + stafferPlayer.getName() + " §3§lTEMPORARIAMENTE §fpelo motivo " + reason + " §firá expirar em " + Longs.messageSmall(expire) + ".");
				}
			}
		} else {
			ProxiedPlayer punidoPlayer = ProxyServer.getInstance().getPlayer(punidoBungee.getPlayer());
			if (stafferPlayer.getName() == stafferPlayer.getName()) {
				stafferPlayer.sendMessage("§c§lPUNISH §fVocê não pode punir a si mesmo.");
				return;
			}
			...
			this.player = punidoPlayer.getName();
			this.ipAddrees = punidoBungee.getGroups().getIpAddrees();
			this.staffer = stafferPlayer.getName();
			this.date = Date.getMonth();
			this.banType = banType;
			...
			if (banType.equals(BanType.BANNED)) {
				...
				if (punidoPlayer != null) {
					punidoPlayer.disconnect(messageOnEntry());
				}
			} else if (banType.equals(BanType.MUTATED)) {
				if (temporaryType.equals(TemporaryType.PERMANENT)) {
					stafferPlayer.sendMessage("§3§lMUTE §fVocê §3§lMUTOU §fo jogador " + stafferPlayer.getName() + " §3§lPERMANENTEMENTE §fpelo motivo " + reason + " §fsem data de expiração.");
	...
	public void chatMessage() {
		for (ProxiedPlayer players : ProxyServer.getInstance().getPlayers()) {
			if (getBanType().equals(BanType.BANNED)) {
				if (BungeeMain.getInstance().hasGroupPermission(players, GroupsType.MODPLUS)) {
