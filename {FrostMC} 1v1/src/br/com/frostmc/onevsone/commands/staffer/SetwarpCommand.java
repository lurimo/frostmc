...
public class SetwarpCommand extends BaseCommand {
	...
	public boolean execute(CommandSender commandSender, String label, String[] args) {
		if (isPlayer(commandSender)) {
			...
			if (args.length == 0) {
				player.sendMessage("§8[§b§lADMINISTRAÇÃO§8] §fUtilize o comando: §7/set (position)");
				return true;
			}
			if (args.length == 1) {
				try {
					...
					player.sendMessage("§8[§b§lADMINISTRAÇÃO§8] §fVocê §a§lSETOU §fo spawn da 1v1.");
				} catch (Exception exception) {
					try {
						Locais locais = Locais.valueOf(args[0].toUpperCase());
						WarpsAPI.setWarpLocation(player, locais);
						player.sendMessage("§8[§b§lADMINISTRAÇÃO§8] §fVocê §a§lSETOU §fa posição do jogador " + locais.name().replaceAll("onevsonepos".toUpperCase(), ""));
					} catch (Exception exception2) {
						player.sendMessage("§8[§b§lADMINISTRAÇÃO§8] §fA warp citada não foi encontrada!");
						return true;
					}
				}
				return true;
